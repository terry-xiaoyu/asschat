...
我很喜欢 MQTT，网络消耗很小，并且它设置了三个 QoS 级别，这一点比起 XMPP 是一个很大的进步。但是在使用市面上的 MQTT Broker 开发 IM 应用时，不够方便和直观，这主要是因为：
- MQTT 协议里考虑 IoT 的应用场景居多，因为它本身就是为 IoT 设计的。
- MQTT v3.1 不够完善。很多现实中的应用场景并没有包含在协议里。

所以我们需要基于 MQTT 基础协议，设计一套上层聊天协议，并根据这套协议对 MQTT broker 做一点定制工作(通过开发插件)。这套上层协议就是 **ASSChat**.

我们的 ASSChat 基于 MQTT v5.0，因为 v5.0 里做了许多有用的改进，比如：
...
### 在线优先 vs. 离线优先：
IM 应用与 IoT 应用给人感觉上非常类似，都是消息传递，但其实两者略有不同。具体来说，主要是对于 `离线` 的处理：
...
### ASSChat Spec 设计原则
- 完全基于 MQTT v5 基础协议。
- 不持久化 Session 和 订阅信息以保持 MQTT Broker 的轻量。定制 `Channel` 以实现 `Offline First`。
...
- Create Channel Request   
  ...
  ```
  reply_topic 的格式是 ass/c/random-generated-id, 每次登录随机生成的。
...
- Create Channel Response  
  ...
  Payload:   
  ```JSON
  {"req-id": id, "rcode": 201, "msg":"channel created", "ch-id": id }
  ```
