...
通过为 Channel 内的所有 Users 订阅相同的 MQTT Topic 来实现聊天，我们称这个 Topic 为 `Channel Topic`。如果 User 有多个终端的话，我们为他的每个终端都订阅 `Channel Topic`，这样我们就实现了多终端之间的消息同步。
...
##### Channel Cmd
如果 Request 里设置了 `"want_reply": true`，服务器需要将回复发布到 `ass/c/{{client-id}}` 主题。Client 上线时，Broker 会为其自动订阅 `ass/c/{{client-id}}`，以接收 ASSChat 协议消息.
